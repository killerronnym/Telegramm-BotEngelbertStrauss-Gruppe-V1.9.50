...
logging.info("Outfit-Bot Skript gestartet (One-Submission-Limit Debug-Version). V4.")
bot = telebot.TeleBot(config.get("BOT_TOKEN", "DUMMY"), threaded=False)

# --- HELPER ---
def is_admin(user_id):
    current_admins = load_json(CONFIG_FILE, DEFAULT_CONFIG).get("ADMIN_USER_IDS", [])
...
def generate_markup(user_id, likes=0, loves=0, fires=0):
    """Erstellt das Tastenfeld mit dynamischen Zählern."""
...
def count_votes(votes_dict):
    """Zählt die Stimmen pro Typ für ein bestimmtes Bild."""
...
def send_daily_post():
    ...
    if not chat_id or chat_id == "DEINE_GRUPPEN_CHAT_ID_HIER": return
    ...
    try:
        ...
        bot.send_message(chat_id, "📸 Outfit des Tages – zeigt eure heutigen E.S-Outfits!", reply_markup=markup)
        logging.info(f"Täglicher Post erfolgreich an {chat_id} gesendet.")
...
def determine_winner():
    ...
    if not bot_data.get("submissions"): 
        if chat_id: 
            try: bot.send_message(chat_id, "Für den heutigen Wettbewerb gab es leider keine Einreichungen.")
            except: pass
        return
    
    winner_info = {}
    max_votes = -1
    for msg_id, votes in bot_data["votes"].items():
    ...
    if not winner_info or max_votes <= 0:
        if chat_id: 
            try: bot.send_message(chat_id, "Es wurden keine Stimmen abgegeben. Es gibt heute keinen Gewinner.")
            except: pass
        reset_contest_data()
        return
    ...
    if winner_user_id:
        try:
            ...
            logging.info(f"Gewinner bekannt gegeben: {username} ({max_votes} Votes).")
...
def handle_photo_submission(message):
    ...
    user_id = str(message.from_user.id) # User ID als String für JSON Keys
    username = message.from_user.username or message.from_user.first_name
    chat_id = load_json(CONFIG_FILE).get("CHAT_ID")

    # Daten vor Prüfung neu laden und debuggen
    bot_data_current = load_json(DATA_FILE)
    logging.info(f"handle_photo_submission: User {user_id}. Aktuelle Submissions: {json.dumps(bot_data_current.get('submissions', {}))}")

    # Prüfung: Hat der User bereits ein Bild eingereicht?
    if user_id in bot_data_current["submissions"]:
        bot.reply_to(message, "🚫 Du hast bereits ein Bild für diesen Wettbewerb hochgeladen.")
        logging.info(f"User {user_id} versucht erneut, Bild hochzuladen. Abgelehnt.")
        return

    photo_id = message.photo[-1].file_id
    caption = f"Outfit von @{username}"
    markup = generate_markup(int(user_id), 0, 0, 0) 
    logging.info(f"Initiales Markup für User {user_id}: {[btn.text for btn in markup.keyboard[0]]}")
    
    try:
        sent_message = bot.send_photo(chat_id, photo_id, caption=caption, reply_markup=markup)
        
        # Daten nach dem Senden aktualisieren und speichern
        bot_data_current["submissions"][user_id] = {"message_id": sent_message.message_id, "photo_id": photo_id, "username": username}
        bot_data_current["votes"][str(sent_message.message_id)] = {}
        save_json(DATA_FILE, bot_data_current)
        
        bot.reply_to(message, "✅ Dein Outfit wurde erfolgreich in der Gruppe gepostet!")
        logging.info(f"Foto von {username} (ID: {user_id}) gepostet. Message ID: {sent_message.message_id}. Submissions nach Update: {json.dumps(bot_data_current.get('submissions', {}))}")
    except Exception as e:
...
def handle_vote(call):
    ...
    bot_data = load_json(DATA_FILE)
    votes = bot_data["votes"].get(message_id, {})
    ...
    if votes.get(voter_id_str) == vote_type:
        ...
        logging.info(f"Voter {voter_id} zieht Stimme {vote_type} für Message {message_id} zurück.")
    else:
        if voter_id_str in votes:
            old_vote_type = votes[voter_id_str]
            logging.info(f"Voter {voter_id} ändert Stimme von {old_vote_type} zu {vote_type} für Message {message_id}.")
        else:
            logging.info(f"Voter {voter_id} gibt neue Stimme {vote_type} für Message {message_id} ab.")
    ...
    logging.info(f"Berechnete Counts für Message {message_id}: {counts}")
    new_markup = generate_markup(submitter_id, counts['like'], counts['love'], counts['fire'])
    
    try:
        logging.info(f"Versuche, Message {message_id} mit Buttons zu aktualisieren: {[btn.text for btn in new_markup.keyboard[0]]}")
        bot.edit_message_reply_markup(chat_id=chat_id, message_id=int(message_id), reply_markup=new_markup)
        logging.info(f"Message {message_id} erfolgreich aktualisiert.")
    except Exception as e:
        if "message is not modified" not in str(e).lower():
            logging.error(f"Fehler beim Update der Buttons für Message {message_id}: {e}", exc_info=True)
        else:
            logging.info(f"Message {message_id} nicht geändert (gleiche Zählerstände oder schnelle Klicks). Debug-Info: {e}")

    try:
        bot.answer_callback_query(call.id, feedback)
    except Exception as e:
        logging.error(f"Callback Answer Error für Message {message_id}: {e}", exc_info=True)
...
# --- ADMIN COMMANDS ---
@bot.message_handler(commands=['start_contest', 'announce_winner', 'set_post_time', 'set_winner_time', 'toggle_auto_post'])
    ...
    while True:
        try:
            if os.path.exists("command_start_contest.tmp"):
                send_daily_post()
                try: os.remove("command_start_contest.tmp")
                except: pass
            if os.path.exists("command_announce_winner.tmp"):
                determine_winner()
                try: os.remove("command_announce_winner.tmp")
                except: pass
            time.sleep(2)
...
def load_schedules():
    ...
    if cfg.get("AUTO_POST_ENABLED", False):
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            if current_token and current_token != "DEIN_TELEGRAM_BOT_TOKEN_HIER":
                if bot.token != current_token: bot.token = current_token
