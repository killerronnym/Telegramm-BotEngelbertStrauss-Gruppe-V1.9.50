...
from shared_bot_utils import get_db_url, get_bot_config, is_bot_active, get_shared_flask_app, log_user_interaction
...
async def letsgo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Startet die Steckbrief-Erstellung."""
    try:
        logger.info(f"DEBUG: letsgo: call start for user={update.effective_user.id}")
        
        # 1. Check Bot Status
        if not is_bot_active('invite'):
            logger.warning(f"DEBUG: letsgo: invite bot is not active.")
            return ConversationHandler.END
        
        # 2. Load Config
        logger.info("DEBUG: letsgo: fetching config...")
        config = get_bot_config('invite')
        if not config or not config.get('is_enabled'):
            logger.info("DEBUG: letsgo: bot is disabled in config.")
            await update.message.reply_text("Der Bot ist zur Zeit deaktiviert.")
            return ConversationHandler.END
        ...
        if not fields:
            logger.info("DEBUG: letsgo: No active fields.")
        ...
        logger.info(f"letsgo: Sende erste Frage: {label}")
        ...
        logger.error(f"CRITICAL: letsgo failed for {update.effective_user.id}: {e}")
        import traceback
        ...
        if whitelist_active:
            ...
            with flask_app.app_context():
                ...
                if existing:
                    ...
                    existing.answers_json = json.dumps(answers) # Nur Antworten speichern!
...
async def handle_existing_member_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with flask_app.app_context():
        ...
        if action == "accept" or action == "resend":
            ...
            if not final_text:
                await query.edit_message_text("❌ Fehler beim Rekonstruieren des Steckbriefs. Daten fehlen.")
                return
