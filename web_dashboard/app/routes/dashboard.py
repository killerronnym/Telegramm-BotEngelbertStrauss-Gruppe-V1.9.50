...
def bot_settings():
    ...
    return render_template("bot_settings.html", config=config, is_invite_running=status['invite']['running'], user_interaction_logs=user_interaction_logs, invite_bot_logs=invite_bot_logs)
...
def add_field():
    ...
    if any(f['id'] == field_id for f in fields):
        ...
        return redirect(url_for('dashboard.bot_settings'))
    new_field = {'id': field_id, 'emoji': request.form.get('emoji', '🔹'), 'display_name': request.form.get('display_name', ''), 'label': request.form.get('label', ''), 'type': request.form.get('type', 'text'), 'required': 'required' in request.form, 'enabled': True}
...
def invite_bot_move_field(field_id, direction):
    ...
    if idx != -1:
        if direction == 'up' and idx > 0: form_fields[idx], form_fields[idx-1] = form_fields[idx-1], form_fields[idx]
        elif direction == 'down' and idx < len(form_fields)-1: form_fields[idx], form_fields[idx+1] = form_fields[idx+1], form_fields[idx]
...
def clear_user_logs():
    if os.path.exists(USER_INTERACTION_LOG_FILE):
        ...
        except Exception as e: flash(f'Fehler beim Löschen der Logs: {e}', 'danger')
    return redirect(url_for('dashboard.bot_settings'))
...
def save_broadcast():
    ...
    if media and media.filename:
        ...
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')): media_type = 'image'
        elif filename.lower().endswith(('.mp4', '.mov')): media_type = 'video'
        else: media_type = 'document'
    scheduled_at = datetime.utcnow()
    if action == 'schedule' and scheduled_at_str:
        try: scheduled_at = datetime.strptime(scheduled_at_str, '%Y-%m-%dT%H:%M')
        except ValueError: pass
    broadcast = Broadcast(text=text, topic_id=topic_id, send_mode=send_mode, media_path=media_path, media_type=media_type, scheduled_at=scheduled_at, status='pending', pin_message=pin_message, silent_send=silent_send)
...
def save_topic_mapping():
    ...
    if topic_id and topic_name:
        try:
            ...
            if mapping: mapping.topic_name = topic_name
            else: mapping = TopicMapping(topic_id=tid_int, topic_name=topic_name); db.session.add(mapping)
            db.session.commit(); flash('Topic gespeichert.', 'success')
        except ValueError: flash('Fehler: Topic ID muss eine Zahl sein.', 'danger')
    else: flash('Fehler: Topic ID und Name erforderlich.', 'danger')
    return redirect(url_for('dashboard.broadcast_manager'))
...
def delete_topic_mapping(topic_id):
    try:
        tid_int = int(topic_id)
        mapping = TopicMapping.query.filter_by(topic_id=tid_int).first()
        if mapping: db.session.delete(mapping); db.session.commit(); flash('Topic gelöscht.', 'success')
    except: pass
    return redirect(url_for('dashboard.broadcast_manager'))
...
def delete_broadcast(broadcast_id):
    ...
    if broadcast: db.session.delete(broadcast); db.session.commit(); flash('Broadcast gelöscht.', 'success')
    return redirect(url_for('dashboard.broadcast_manager'))
...
def quiz_settings():
    ...
    if not settings: settings = BotSettings(bot_name='quiz', config_json='{}'); db.session.add(settings); db.session.commit()
    config = json.loads(settings.config_json)
    if request.method == 'POST':
        action = request.form.get('action')
        if action == 'save_settings': config['bot_token'] = request.form.get('token'); config['channel_id'] = request.form.get('channel_id'); config['topic_id'] = request.form.get('topic_id')
        elif action == 'save_schedule': config['schedule'] = {'enabled': 'schedule_enabled' in request.form, 'time': request.form.get('schedule_time'), 'days': [int(d) for d in request.form.getlist('schedule_days')]}
        settings.config_json = json.dumps(config); db.session.commit(); return redirect(url_for('dashboard.quiz_settings'))
    schedule = config.get('schedule', {'enabled': False, 'time': '12:00', 'days': []})
    return render_template('quiz_settings.html', schedule=schedule, stats={'total': 0, 'asked': 0, 'remaining': 0}, config=config, questions_json='[]', asked_questions_json='[]', logs=[])
...
def umfrage_settings():
    ...
    if not settings: settings = BotSettings(bot_name='umfrage', config_json='{}'); db.session.add(settings); db.session.commit()
    config = json.loads(settings.config_json)
...
def outfit_bot_dashboard():
    ...
    if not settings: settings = BotSettings(bot_name='outfit', config_json='{}'); db.session.add(settings); db.session.commit()
    config = json.loads(settings.config_json)
...
def outfit_bot_actions(action): return redirect(url_for('dashboard.outfit_bot_dashboard'))
...
def get_id_finder_settings():
    ...
    if not settings:
        initial_config = {'bot_token': '', 'admin_group_id': 0, 'main_group_id': 0, 'admin_log_topic_id': None, 'delete_commands': True, 'bot_message_cleanup_seconds': 60, 'message_logging_enabled': True, 'message_logging_ignore_commands': True, 'message_logging_groups_only': False, 'max_warnings': 3, 'punishment_type': 'none', 'mute_duration': 24, 'cleanup_notification_seconds': 60, 'warning_bot_name': 'invite'}
        settings = BotSettings(bot_name='id_finder', config_json=json.dumps(initial_config)); db.session.add(settings); db.session.commit()
    return settings
...
def id_finder_dashboard():
    ...
    if os.path.exists(ID_FINDER_BOT_LOG_FILE):
         try:
            with open(ID_FINDER_BOT_LOG_FILE, 'r', encoding='utf-8') as f: logs = f.readlines()[-50:]
         except: pass
    return render_template('id_finder_dashboard.html', config=config, user_registry=users, is_running=status['id_finder']['running'], logs=logs)
...
def id_finder_save_config():
    ...
    # Neu: Moderations-Einstellungen
    try: config['max_warnings'] = int(request.form.get('max_warnings', 3))
    except: pass
    config['punishment_type'] = request.form.get('punishment_type', 'none')
    try: config['mute_duration'] = int(request.form.get('mute_duration', 24))
    except: pass
    try: config['cleanup_notification_seconds'] = int(request.form.get('cleanup_notification_seconds', 60))
    except: pass
    config['warning_bot_name'] = request.form.get('warning_bot_name', 'invite')

    settings.config_json = json.dumps(config); db.session.commit(); flash('Konfiguration gespeichert.', 'success')
...
def id_finder_admin_panel():
    ...
    available_permission_groups = {"Basis-Moderation": {"can_warn": "Nutzer warnen", "can_mute": "Nutzer stummschalten", "can_ban": "Nutzer bannen"}, "Nachrichten-Management": {"can_delete": "Nachrichten löschen", "can_purge": "Chat leeren (Purge)"}, "System": {"can_broadcast": "Broadcasts senden", "can_manage_admins": "Admins verwalten"}}
    return render_template('id_finder_admin_panel.html', admins=admins, available_permission_groups=available_permission_groups)

@bp.route('/id-finder/admin-panel/add', methods=['POST'])
def id_finder_add_admin():
    admin_id, admin_name = request.form.get('admin_id'), request.form.get('admin_name')
    if not admin_id or not admin_name: flash('ID und Name sind erforderlich.', 'danger'); return redirect(url_for('dashboard.id_finder_admin_panel'))
    try:
        telegram_id = int(admin_id); existing = IDFinderAdmin.query.filter_by(telegram_id=telegram_id).first()
        if existing: flash('Admin mit dieser ID existiert bereits.', 'warning')
        else: new_admin = IDFinderAdmin(telegram_id=telegram_id, name=admin_name); db.session.add(new_admin); db.session.commit(); flash('Admin erfolgreich hinzugefügt.', 'success')
    except ValueError: flash('Ungültige Telegram ID.', 'danger')
...
def id_finder_delete_admin():
    admin_id = request.form.get('admin_id'); admin = IDFinderAdmin.query.filter_by(telegram_id=admin_id).first()
    if admin: db.session.delete(admin); db.session.commit(); flash('Admin gelöscht.', 'success')
    else: flash('Admin nicht gefunden.', 'danger')
    return redirect(url_for('dashboard.id_finder_admin_panel'))

@bp.route('/id-finder/admin-panel/update', methods=['POST'])
def id_finder_update_admin_permissions():
    admin_id = request.form.get('admin_id'); admin = IDFinderAdmin.query.filter_by(telegram_id=admin_id).first()
    if not admin: flash('Admin nicht gefunden.', 'danger'); return redirect(url_for('dashboard.id_finder_admin_panel'))
    permissions = {key: True for key in request.form.keys() if key != 'admin_id'}
    admin.permissions = permissions; db.session.commit(); flash(f'Berechtigungen für {admin.name} aktualisiert.', 'success')
    return redirect(url_for('dashboard.id_finder_admin_panel'))

@bp.route('/id-finder/analytics')
def id_finder_analytics():
    days = request.args.get('days', type=int); month = request.args.get('month', 0, type=int); year = request.args.get('year', 0, type=int); query = IDFinderMessage.query
    if days: start_date = datetime.utcnow() - timedelta(days=days); query = query.filter(IDFinderMessage.timestamp >= start_date)
    if month: query = query.filter(func.strftime('%m', IDFinderMessage.timestamp) == f'{month:02d}')
    if year: query = query.filter(func.strftime('%Y', IDFinderMessage.timestamp) == str(year))
    messages = query.all(); total_users = IDFinderUser.query.count(); total_messages = len(messages); busiest_hours = [0] * 24; busiest_days = [0] * 7
    for m in messages: busiest_hours[m.timestamp.hour] += 1; busiest_days[m.timestamp.weekday()] += 1
    labels, total_timeline = [], []
    d_range = days if days else 7
    for i in range(d_range): date = (datetime.utcnow() - timedelta(days=d_range-1-i)).date(); labels.append(date.strftime('%d.%m.')); total_timeline.append(sum(1 for m in messages if m.timestamp.date() == date))
    user_stats = {}
    for m in messages:
        uid = m.telegram_user_id
        if uid not in user_stats: user = IDFinderUser.query.filter_by(telegram_id=uid).first(); user_stats[uid] = {'uid': uid, 'name': user.first_name if user else str(uid), 'msgs': 0, 'media': 0}
        user_stats[uid]['msgs'] += 1
        if m.content_type != 'text': user_stats[uid]['media'] += 1
    leaderboard = sorted(user_stats.values(), key=lambda x: x['msgs'], reverse=True)[:20]
    return render_template('id_finder_analytics.html', stats={'total_users': total_users, 'total_messages': total_messages}, activity={'timeline': {'labels': labels, 'total': total_timeline}, 'busiest_hours': busiest_hours, 'busiest_days': busiest_days, 'leaderboard': leaderboard})

@bp.route('/api/id-finder/user-activity/<int:uid>')
def id_finder_user_activity_api(uid):
    days = request.args.get('days', 7, type=int); timeline = []
    for i in range(days): date = (datetime.utcnow() - timedelta(days=days-1-i)).date(); count = IDFinderMessage.query.filter(IDFinderMessage.telegram_user_id == uid, func.date(IDFinderMessage.timestamp) == date).count(); timeline.append(count)
...
def id_finder_user_detail(user_id):
    ...
    if not user: flash(f'User {user_id} nicht gefunden.', 'danger'); return redirect(url_for('dashboard.id_finder_dashboard'))
...
def id_finder_delete_user(user_id):
    user = IDFinderUser.query.filter_by(telegram_id=user_id).first()
    if user: db.session.delete(user); db.session.commit(); flash(f'User {user_id} wurde gelöscht.', 'success')
    return redirect(url_for('dashboard.id_finder_dashboard'))
...
def minecraft_status_page():
    class Config:
        def __init__(self): self.mc_host, self.mc_port, self.display_host, self.display_port, self.chat_id, self.topic_id, self.update_seconds, self.delete_player_seconds, self.status_message_id, self.host, self.port = '', 25565, '', 25565, '', '', 30, 8, '', '', ''
    return render_template('minecraft_status.html', cfg=Config(), latency_ms=None, motd=None, players_text=None, error=None, log_paths=[], log_tail='')
...
def add_user():
    ...
    if username and password:
        if User.query.filter_by(username=username).first(): flash('Benutzer existiert bereits', 'danger')
        else: user = User(username=username, role=role); user.set_password(password); db.session.add(user); db.session.commit(); flash('Benutzer angelegt', 'success')
    return redirect(url_for('dashboard.manage_users'))

@bp.route('/users/delete/<username>', methods=['POST'])
def delete_user(username):
    user = User.query.filter_by(username=username).first()
    if user: db.session.delete(user); db.session.commit(); flash('Benutzer gelöscht', 'success')
    return redirect(url_for('dashboard.manage_users'))
...
def edit_user(username):
    user = User.query.filter_by(username=username).first()
    if user:
        new_username, new_password, new_role = request.form.get('new_username'), request.form.get('new_password'), request.form.get('new_role')
        if new_username and new_username != username: user.username = new_username
        if new_password: user.set_password(new_password)
        if new_role: user.role = new_role
        db.session.commit(); flash('Benutzer aktualisiert', 'success')
    return redirect(url_for('dashboard.manage_users'))
